...
export function triggerEvent(): void {
  ...
  if (ev.id === 'pluie') {
    ...
    for (let i = 0; i < 16; i++) {
      setTimeout(() => {
        if (state.runId === thisRun && !state.gameOver) spawnCat();
      }, i * 160);
    }
  }
  if (ev.id === 'boss') {
    const bossType = (CAT_TYPES as any[]).find(t => t.id === 'boss');
    const thisRun = state.runId;
    setTimeout(() => {
      if (state.runId === thisRun && !state.gameOver && bossType) spawnCat(bossType);
    }, 1800);
    Music.sfxLevelUp();
  }
}
