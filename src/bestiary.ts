...
try {
  ...
  for (const id of saved) caughtTypes.add(id);
} catch { /* invalid JSON, ignore */ }

export function markCaught(typeId: string): void {
  if (caughtTypes.has(typeId)) return;
  caughtTypes.add(typeId);
  localStorage.setItem('pdc_caught', JSON.stringify([...caughtTypes]));
}

export function drawCatPreview(cvs: HTMLCanvasElement, type: CatType, caught: boolean): void {
  ...
  if (type.stripes) {
    ...
    for (let i = -1; i <= 1; i++) {
      tc.beginPath();
      tc.moveTo(cx+i*s*.28, cy-s*.58);
      tc.lineTo(cx+i*s*.22, cy-s*.1);
      tc.stroke();
    }
  }
  ...
  tc.fillStyle = '#ff9eb5';
  tc.beginPath();
  tc.moveTo(cx, cy+s*.12);
  tc.lineTo(cx-s*.07, cy+s*.22);
  tc.lineTo(cx+s*.07, cy+s*.22);
  tc.fill();
  ...
  if (type.id === 'fantome') {
    tc.globalAlpha = .38;
    tc.fillStyle = '#fff';
    tc.beginPath(); tc.arc(cx, cy, s*.7, 0, Math.PI*2); tc.fill();
    tc.globalAlpha = 1;
  }
  if (type.id === 'rainbow') {
    const rg = tc.createLinearGradient(cx-s, cy, cx+s, cy);
    ['#f00','#ff0','#0f0','#00f','#f0f'].forEach((c, i, a) => rg.addColorStop(i/(a.length-1), c));
    tc.globalAlpha = .3;
    tc.fillStyle = rg;
    tc.beginPath(); tc.arc(cx, cy, s*.7, 0, Math.PI*2); tc.fill();
    tc.globalAlpha = 1;
...
export function buildBestiary(): void {
  const grid = document.getElementById('bst-grid');
  if (!grid) return;
  grid.innerHTML = '';

  const found = (CAT_TYPES as CatType[]).filter(t => caughtTypes.has(t.id)).length;
...
export function openBestiaryDetail(type: CatType): void {
  ...
  const ptsEl = document.getElementById('bst-detail-pts');
  if (ptsEl) ptsEl.textContent = (type.pts > 0 ? '+' : '') + type.pts + ' pts';
