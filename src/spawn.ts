// @ts-nocheck
import { state, cats, _catIdCounter as _ } from './state';
...
export function spawnCat(forcedType) {
  ...
  cats.push({
    ...
    bossAttackTimer: 0,
    bossAttackKind: 0,
    bossAnchorX: 0, bossWobble: Math.random() * Math.PI * 2,
    ...
    speech: (()=>{
      ...
      return { text: lines[Math.floor(Math.random()*lines.length)], age: 0, dur: 2.2 + Math.random()*1.2 };
    })(),
  });
}

export function pickObjectType() {
  const total = OBJECT_TYPES.reduce((s,t) => s + (t.w||1), 0);
  let r = Math.random() * total;
  for (const t of OBJECT_TYPES) { r -= (t.w||1); if (r <= 0) return t; }
...
export function pickCatType(forceRare) {
  let pool = CAT_TYPES.filter(t => t.minLvl <= state.level && t.w > 0);
